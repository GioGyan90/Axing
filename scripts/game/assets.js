...
function createVoxelBody(radius, height, material, lowerBodyMaterial = material) {
    ...
    // Abdomen section (between torso and hips).  It extends below the old
    // waist line so the shirt fills the empty space under the large head.
    const abdomenWidth = torsoWidth * 0.92;
    const abdomenHeight = height * 0.42;
    ...
    abdomen.position.y = -height * 0.02;
    abdomen.castShadow = true;
    body.add(abdomen);
    
    // Hips / waist section, pulled downward to bridge the gap to the legs.
    const hipWidth = torsoWidth * 0.9;
    const hipHeight = height * 0.42;
    const hipDepth = torsoDepth * 0.94;
    const hips = new THREE.Mesh(new THREE.BoxGeometry(hipWidth, hipHeight, hipDepth), lowerBodyMaterial);
    hips.position.y = -height * 0.38;
    hips.castShadow = true;
    body.add(hips);
    // Rear hip/butt block gives the lower body volume from side/back views.
    const rearHip = new THREE.Mesh(new THREE.BoxGeometry(hipWidth * 0.86, hipHeight * 0.92, hipDepth * 0.44), lowerBodyMaterial);
    rearHip.position.set(0, -height * 0.4, -hipDepth * 0.36);
    rearHip.castShadow = true;
    body.add(rearHip);

    
    // Groin/crotch piece (small connector between legs), extended downward so
    // the pelvis reaches the top of the thighs instead of floating above them.
    const groinWidth = hipWidth * 0.58;
    const groinHeight = height * 0.36;
    const groinDepth = hipDepth * 0.78;
    const groin = new THREE.Mesh(new THREE.BoxGeometry(groinWidth, groinHeight, groinDepth), lowerBodyMaterial);
    groin.position.y = -height * 0.68;
